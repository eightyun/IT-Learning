...
public class RefundApplyServiceImpl extends ServiceImpl<RefundApplyMapper, RefundApply> implements IRefundApplyService {
    ...
    public void handleRefundResult(RefundResultDTO result) {
        ...
        // 2.判断结果，支付宝可能直接返回退款成功结果 ， 微信指挥返回退款中
        RefundApply newrefundApply = new RefundApply();
        newrefundApply.setId(refundApply.getId()) ;
        newrefundApply.setRefundChannel(refundApply.getRefundChannel());
        newrefundApply.setRefundOrderNo(refundApply.getRefundOrderNo());
        // 2.1判断状态是否退款
        int status = result.getStatus() ;
        if(status == RefundResultDTO.RUNNING) ;
        {
            updateById(newrefundApply) ;
            return ;
        }
        // 2.2判断退款是否成功
        ...
        {
            updateById(newrefundApply) ;
            newrefundApply.setStatus(RefundStatus.SUCCESS.getValue()) ;
            newrefundApply.setMessage(RefundStatus.SUCCESS.getProgressName()) ;
        }
        else
        {
            // 2.3退款失败，记录状态及其退款失败原因
            newrefundApply.setStatus(RefundStatus.FAILED.getValue()) ;
            newrefundApply.setMessage(RefundStatus.FAILED.getProgressName()) ;
            newrefundApply.setFailedReason(result.getMsg()) ;
        }
        // 2.4更新数据库
        newrefundApply.setFinishTime(LocalDateTime.now()) ;
        updateById(newrefundApply) ;
        // 3.更新子订单状态
        detailService.updateRefundStatusById(refundApply.getOrderDetailId() , newrefundApply.getStatus());
        // 4.退款成功 取消用户报名课程
        if(status == RefundResultDTO.SUCCESS)
        {
            // 4.1查询子订单信息
            OrderDetail detail = detailService.getById(newrefundApply.getOrderDetailId());
            // 4.2发送mq消息 通知报名成功
            rabbitMqHelper.send(
