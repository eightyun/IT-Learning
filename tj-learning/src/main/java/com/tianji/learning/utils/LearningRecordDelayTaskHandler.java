...
public class LearningRecordDelayTaskHandler {
    ...
    //创建线程池
    ThreadPoolExecutor poolExecutor = new ThreadPoolExecutor(16 ,
            20,
            60,
            TimeUnit.SECONDS,
    ...
    //PostConstruct：在此类初始化后，并在属性被输入之后执行 一般用来做一些初始化的工作
    @PostConstruct
    public void init()
    {
            log.info("LearningRecordDelayTaskHandler init方法执行了");
            // 开启新线程执行handleDelayTask
    ...
    private void handleDelayTask(){
        while (begin){
            try {
                // 1.尝试获取任务
                DelayTask<RecordTaskData> task = queue.take();// 从延迟阻塞队列拉去任务  是阻塞方法 一直等
